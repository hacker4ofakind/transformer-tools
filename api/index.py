...
app = FastAPI(title="Item CRUD API", docs_url="/api/docs", openapi_url="/api/openapi.json")
...
@app.get("/api/items/{item_name}", response_model=Item)
def read_item(item_name: str):
    if item_name not in items_db:
        raise HTTPException(status_code=404, detail="Item not found")
    return items_db[item_name]

@app.put("/api/items/{item_name}", response_model=Item)
